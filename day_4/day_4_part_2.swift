...
extension Day_4_Part_2 {
    
    static func solution(_ source: Input) throws -> Int {
        var input = try Self.parseInput(source)
        
        var result = 0
        var removedLast = -1
        while removedLast != 0 {
            removedLast = countAndRemove(in: &input)
            result += removedLast
        }
    ...
    private static func countAndRemove(in input: inout [[String]]) -> Int {
...
let result = try Day_4_Part_2.solution(.main)
print(result)